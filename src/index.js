...
function initModules() {
  ...
  if (!cacheManager) {
    ...
    // Register caches with TTL
    cacheManager.registerCache('routing', 5 * 60 * 1000); // 5 minutes
    cacheManager.registerCache('skills', 10 * 60 * 1000); // 10 minutes
  }
...
module.exports = {
  ...
  init: async(options = {}) => {
    ...
    const skillNames = skillDiscovery.discoverSync();
  ...
  getModelWithFallback: (agentType, taskDescription) => {
    ...
    // Check cache first
    const cacheKey = `routing:${agentType}:${taskDescription}`;
    if (cacheManager.has('routing', cacheKey)) {
      return cacheManager.get('routing', cacheKey);
    }

    const config = configLoader.loadSync();
    const result = modelRouter.getModelWithFallback(config, agentType, taskDescription);

    // Cache the result
    if (result) {
      cacheManager.set('routing', cacheKey, result, 5 * 60 * 1000);
    }
